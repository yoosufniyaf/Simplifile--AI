...
async def export_report(report_type: str, format: str = "csv", user: dict = Depends(get_current_user)):
    ...
    if report.report_type == "profit-loss":
        summary_rows = [
            ("Revenue", money(data.get("revenue", 0))),
            ("Total Expenses", money(data.get("total_expenses", 0))),
            ("Net Income", money(data.get("net_income", 0))),
        ]
    elif report.report_type == "balance-sheet":
        summary_rows = [
            ("Total Assets", money(data.get("total_assets", 0))),
            ("Total Liabilities", money(data.get("total_liabilities", 0))),
            ("Equity", money(data.get("equity", 0))),
        ]
    elif report.report_type == "cash-flow":
        summary_rows = [
            ("Cash In", money(data.get("cash_in", 0))),
            ("Cash Out", money(data.get("cash_out", 0))),
            ("Net Cash Flow", money(data.get("net_cash_flow", 0))),
        ]
    ...
    for key, value in data.items():
        if isinstance(value, dict) and value:
            nested_sections.append((pretty_label(key), value))

    for section_title, section_data in nested_sections:
        y -= 15
        if y < 100:
            pdf.showPage()
            y = height - 50
        ...
        for item_key, item_value in section_data.items():
            ...
            pdf.drawString(60, y, pretty_label(item_key))
    ...
    pdf.drawString(50, y, "Generated by Simplifile AI")
